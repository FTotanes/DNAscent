'''Scripts have been written to run on Python 3'''
...
def convert_bedfile(location_filename):
    '''Convert DNAscent files to bedfiles with chromosome IDs converted to PF3D7_01_v3 ... PF3D7_14_v3 format.
    ...
    convert=list(map(lambda x:[convert_chr(x[0]),int(x[1]),int(x[2]),x[3],x[7]],a))
    convert=sorted(convert,key=lambda x:(x[0],x[1]))
    convert='\n'.join(list(map(lambda x:'\t'.join(map(lambda y:str(y),x)),convert)))
    return convert

def merge_bedfiles(filename_list):
    '''This function can be used to merge bedfiles of runs from the same timepoint.
    Space separated list of the complete path for each file to be merged is required ("filename_list").
    ...
    for filename in filename_list:
        ...
        bedfile=list(map(lambda x:[convert_chr(x[0]),int(x[1]),int(x[2]),x[3],x[4]],a))
        merged_bed+=bedfile
    merged_bed=sorted(merged_bed,key=lambda x:(x[0],x[1]))
    merged_bed='\n'.join(list(map(lambda x:'\t'.join(map(lambda y:str(y),x)),merged_bed)))
    return merged_bed
